...
from numpy import cumsum, exp, sqrt, array, asarray
...
def dreamzPTStep(func, priorFunc, \
                     ...
                     ncr=0, navg=1, \
                     eps=1.e-6, e=0.05, \
                     fitVector=array([]), args=array([]), plow=array([]), phigh=array([]), thin=10):
...
def dreamzStep(func, priorFunc, \
                   ...
                   fitVector=array([]), args=array([]), plow=array([]), phigh=array([])):
...
def dreamStep(func, priorFunc, \
                   ...
                   ncr=0, navg=1, \
                   gamma=1.0, eps=1.e-6, e=0.05, \
                   fitVector=array([]), args=array([]), violateDB=False):
...
def snookerStep(func, priorFunc, \
                    X, XP, Z, \
                    fitVector=array([-1]), args=array([-1])):
    nChains, ndim = shape(X)
    M, nparms = shape(Z)
    XNEW = copy(X)
    XPNEW = copy(XP)
