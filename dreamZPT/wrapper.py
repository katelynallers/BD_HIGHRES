...
def DREAM(X, logl, logp, niter, thin=1, ncr=0, navg=1, fitVector=np.array([]), \
              args=np.array([])):
    '''
...
def DREAMZ(ZZ, logl, logp, nchains, niter, thin=1, ncr=0, navg=1, fitVector=np.array([]), \
              args=np.array([])):
    '''
...
def DREAMZS(ZZ, logl, logp, nchains, niter, thin=1,\
            ncr=0, navg=1, \
            fitVector=np.array([]), \
            args=np.array([]), verbose=True, snookerProb=0.1, \
            plow=np.array([]), phigh=np.array([])):
    '''
    ZZ = initial guess at parameters; shape = (ninit, ndim)
...
def DREAMZPT(ZZ, logl, logp, XT, niter, thin=1, ncr=0, navg=1, \
                fitVector=np.array([]), \
                args=np.array([]), verbose=True,plow=np.array([]), phigh=np.array([])):
    '''
    ZZ = initial guess at parameters; shape = (ninit, ndim)
    ...
    for i in range(niter):
        ...
        X, XP, success = dreamzPTStep(logl, logp, X, XP, XT, Z[0:nsofar],\
                                            ...
                                            ncr=ncr, navg=navg, \
                                            fitVector=fitVector, \
                                            args=args, plow=np.array([]), phigh=np.array([]), \
                                            thin=thin)
        successVec += success
